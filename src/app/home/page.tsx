...
export default function HomePage() {
  ...
  async function getPosts() {
    ...
    const { data: postUser3 } = await supabase
      .from('posts')
      .select('*')
      .eq("passid", "LGS-49fb3e")
      .order('created_at', { ascending: false });
    if (postUser2 != null) {
      for (let g2 = 0; g2 < postUser2?.length; g2++) {
        postArray.push(postUser2[g2]);
      }
    }
    if (postUser3 != null) {
      for (let g2 = 0; g2 < postUser3?.length; g2++) {
        postArray.push(postUser3[g2]);
      }
    }
    ...
    if (subs != null) {
      for (let g1 = 0; g1 < subs?.length; g1++) {
        if (subs[g1]?.passid2 != "LGS-P7nX10" && subs[g1]?.passid2 != "LGS-49fb3e") {
          const { data: postUser } = await supabase
            .from('posts')
            .select('*')
            .eq("passid", subs[g1]?.passid2)
            .order('created_at', { ascending: false });
          if (postUser != null) {
            for (let g2 = 0; g2 < postUser?.length; g2++) {
              postArray.push(postUser[g2]);
            }
          }
        }
      }
  ...
  return (
    <main className='bg-dark'>
      ...
      <section className='bg-dark min-w-screen min-h-screen py-4 mx-auto text-white xl:w-[1280px]'>
        <Header passport={{ userID: authData?.id, authData: authData }} />
