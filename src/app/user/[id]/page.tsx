...
export default function HomePage({ params }: { params: { id: string } }) {
  ...
  return (
    <main className='bg-dark'>
      ...
      {givePassport ?
        <div className='z-50 absolute left-0 top-0 backdrop-blur-[3px] w-full h-full text-white flex justify-center items-center' onClick={(e) => {
          setGivePassport(false);
        }}>
          <div className='py-4 px-6 border border-zinc-700 bg-dark rounded-2xl w-1/3 flex flex-col items-center' onClick={(e) => {
            e.stopPropagation();
          }}>
